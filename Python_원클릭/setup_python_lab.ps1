...
Write-Host ""
Write-Host "=== Python IDLE 바로가기 생성 ==="

$idleExe = (Get-Command py).Source

$shortcutTargets = @(
    (Join-Path $PSScriptRoot "Python IDLE.lnk"),
    (Join-Path ([Environment]::GetFolderPath("Desktop")) "Python IDLE.lnk")
)
...
foreach ($shortcutPath in $shortcutTargets) {
    ...
    $shortcut.TargetPath = $idleExe
    $shortcut.Arguments = "-m idlelib"
    $shortcut.WorkingDirectory = $HOME
    $shortcut.IconLocation = "$idleExe,0"
    $shortcut.Save()
...
$Dialog.Description = "PyCharm Python 실습 폴더를 선택하세요."
