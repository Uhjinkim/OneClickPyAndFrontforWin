...
Write-Host ""
Write-Host "=== Visual Studio Code 바로가기 생성 ==="

$codeCmd = (Get-Command code).Source
$codeExe = (Resolve-Path (Join-Path (Split-Path $codeCmd -Parent) "..\Code.exe")).Path

$shortcutTargets = @(
    (Join-Path $PSScriptRoot "Visual Studio Code.lnk"),
    (Join-Path ([Environment]::GetFolderPath("Desktop")) "Visual Studio Code.lnk")
)
...
function Select-ProjectFolder {
    ...
    try {
        ...
        $dialog.Description = "바이브코딩 실습 폴더를 선택하세요."
...
if ([string]::IsNullOrWhiteSpace($ProjectFolder)) {
    ...
    Write-Host ""
    Write-Host "실습 폴더가 선택되지 않았습니다." -ForegroundColor Red
